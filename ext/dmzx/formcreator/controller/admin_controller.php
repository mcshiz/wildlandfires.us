	...
	*/
	...
	public function display_options()
	{
		...
		if ($update)
		{
			...
			{
				$data = array(
				    'mandatory' => isset($post['q_mandatory'][$key]) ? '1' : '0',
                    'format_buttons' => isset($post['q_format_buttons'][$key]) ? '1' : '0',
                    );
		...
		if ($addnew)
		{
			...
			$sql_ary = array(
				...
				'format_buttons' => ($this->request->is_set_post('format_buttons') ? '1' : '0')
			);
		...
		// main sql //
		$sql = 'SELECT id, form_id, ndx_order, name, hint, type, mandatory, options, forum_name, forum_id, format_buttons
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$disabled = '';
			$checked = '';
			$format_buttons_checked = '';

			if ($row['mandatory'])
			{
				$checked = 'checked="checked"';
			}
            if ($row['format_buttons'])
            {
                $format_buttons_checked = 'checked="checked"';
            }
			...
			$this->template->assign_block_vars('form_template', array(
				...
				'FORMAT_BUTTONS'=> $row['format_buttons'] ,
				'OPTIONS'		=> $row['options'] ,
				'CHECKED'		=> $checked ,
				'FORMAT_BUTTONS_CHECKED' => $format_buttons_checked,
				'ID'			=> $row['id'] ,
	...
	function build_preview($form_id)
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			switch (strtolower($row['type']))
			{
				...
				case 'text':
				case 'file':
					$type = '<input style="border-radius: 5px; width:300px" class="text" type="' . $row['type']. '" name="templatefield_' . $row['name'] . $file_count . '" placeholder="' . $row['hint'] . '" size="40" maxlength="255" tabindex="' . $tabIndex . '" />';
				break;

				case 'textbox':
					$type = '<textarea style="border-radius: 5px;" class="text" name="templatefield_' . $row['name'] . '" rows="3" cols="76" tabindex="' . $tabIndex . '" onselect="storeCaret(this);" onclick="storeCaret(this);" placeholder="' . $row['hint'] . '" onkeyup="storeCaret(this);"></textarea>';
				break;
				case 'selectbox':
					$type = '<select style="border-radius: 5px;" class="inputbox" name="templatefield_' . $row['name'] . '" tabindex="' . $tabIndex . '">';
