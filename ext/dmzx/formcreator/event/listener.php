...
class listener implements EventSubscriberInterface
{
	...
	*/
	...
	private function build_form($forum_id)
	{
		...
		$style = 'style="border:1px solid darkgray; line-height:25px"';
		$style_ta = 'style=""';
		$entry = "";
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
            ...
            $tindx = $row['ndx_order']+2;
			$temp_name = $row['name'];
            // not sure why we replaceing spaces with underscores in the name commented out BJM
	...
	private function grab_form_data($forum_id)
	{
		...
		{
			$name = "";
//			not sure why we were replacing spaces with underscores; commented out BJM
			$row['name'] = str_replace(' ', '_', $row['name']);
